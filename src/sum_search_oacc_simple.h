#ifndef SUM_SEARCH_OACC_SIMPLE_H
#define SUM_SEARCH_OACC_SIMPLE_H

#include "scan_config.cuh"

namespace partial_openacc_simple {
...
#pragma acc parallel deviceptr(g_idata, g_odata, g_aggregates) num_workers(BLOCK_SIZE)
#pragma acc loop gang
  for (int bid = 0; bid < NBLOCKS(N, chunk_size); ++bid) {
    T s_data[SMEM_PER_BLOCK<sizeof(T)>()];
    const size_t block_offset = bid * chunk_size;
#pragma acc loop cache(s_data) worker
    ...
    // Break into separate loops to get around lack of __syncthreads()
    int offset = 1;
#pragma unroll
...
#pragma acc loop cache(s_data) worker
    for (int tid = 0; tid < BLOCK_SIZE; ++tid) {
      // Need to recalculate indices since loops were split
      int ai = tid;
      int bi = tid + chunk_size/2;
      int bank_offset_a = CONFLICT_FREE_OFFSET<sizeof(T)>(ai);
      int bank_offset_b = CONFLICT_FREE_OFFSET<sizeof(T)>(bi);
      ...
      if (tid == BLOCK_SIZE - 1) g_aggregates[bid] = s_data[bi + bank_offset_b];
...
#pragma acc serial deviceptr(tree, agg_begin, found)
  {
...
void sum_search(double r, const T* vec_in_d, T* vec_working_d, void* extra_mem_d, const size_t numel, size_t* result_d) {
  ...
  scan_up_sweep<chunk_size>(vec_in_d, vec_working_d, agg_d, numel);
  while (N > 1) {
    size_t this_N = N;
    T* last_agg_d = agg_d;
    agg_d = reinterpret_cast<T*>(reinterpret_cast<uint8_t*>(agg_d) + 32 + (N*sizeof(T)+31)/32*32);
    N = NBLOCKS(N, chunk_size);
    scan_up_sweep<chunk_size>(last_agg_d, last_agg_d, agg_d, this_N);
  }
  search_tree_device<chunk_size>(r, vec_working_d, numel, agg_d, result_d);
}
